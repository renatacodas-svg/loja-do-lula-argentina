...
type Tab = "dashboard" | "products" | "posts" | "publications" | "orders" | "packs" | "sales_points" | "contacts" | "setup";
...
type SalesPointRequestRow = {
  ...
  support_mode: string;
  can_coordinate_delivery: string;
  can_coordinate_orders_payments: string;
  notes?: string;
  status: string;
  internal_notes?: string;
  created_at: string;
...
export function AdminShell({ mode = "nucleus" }: { mode?: AdminMode }) {
  ...
  const [salesPointRequests, setSalesPointRequests] = useState<SalesPointRequestRow[]>([]);
  ...
  async function refresh() {
    ...
    if (mode === "store") {
      const [productResult, orderResult, packResult, salesPointResult] = await Promise.all([
        supabase.from("products").select("*").order("created_at", { ascending: false }),
        supabase.from("orders").select("*").order("created_at", { ascending: false }),
        supabase.from("pack_requests").select("*").order("created_at", { ascending: false }),
        supabase.from("sales_point_requests").select("*").order("created_at", { ascending: false })
      ]);
      ...
      if (salesPointResult.data) setSalesPointRequests(salesPointResult.data as SalesPointRequestRow[]);
      ...
      if (salesPointResult.error) failedSections.push("pontos de venda");
    } else {
  ...
  async function updateSalesPointRequest(id: string, status: string, internal_notes: string) {
    setMessage("");
    if (!supabase) return;
    const { error } = await supabase.from("sales_point_requests").update({ status, internal_notes }).eq("id", id);
    if (error) {
      setMessage(friendlyError(error, "pedido de ponto de venda"));
      return;
    }
    setSalesPointRequests((current) => current.map((request) => request.id === id ? { ...request, status, internal_notes } : request));
    setMessage("Pedido de ponto de venda atualizado com sucesso.");
  }
  ...
  return (
    <section className="mx-auto max-w-6xl px-4 py-10">
      ...
      <div className="mb-6 flex flex-wrap gap-2">
        {(mode === "store"
          ? (["dashboard", "products", "orders", "packs", "sales_points", "setup"] as const)
          ...
          <button key={tab} onClick={() => setActive(tab)} className={`rounded-md px-4 py-2 text-sm font-black ${active === tab ? "bg-lulaRed text-white" : "bg-white"}`}>
            {tab === "dashboard" ? "Dashboard" : tab === "products" ? "Produtos" : tab === "posts" ? "Atividades" : tab === "publications" ? "Publicações" : tab === "orders" ? "Reservas" : tab === "packs" ? "Pedidos de packs" : tab === "sales_points" ? "Pontos de venda" : tab === "contacts" ? "Mensagens" : "Configuração"}
          </button>
        ))}
      </div>
      ...
      {active === "sales_points" ? <SalesPointRequestsAdmin requests={salesPointRequests} updateRequest={updateSalesPointRequest} /> : null}
...
function SalesPointRequestsAdmin({ requests, updateRequest }: { requests: SalesPointRequestRow[]; updateRequest: (id: string, status: string, notes: string) => void }) {
  if (!requests.length) return <div className="rounded-lg bg-white p-6 shadow-soft">Ainda não há pedidos de pontos de venda no banco.</div>;
  const statusLabels: Record<string, string> = { novo: "Novo", em_contato: "Em contato", encerrado: "Encerrado" };
    ...
    <div className="grid gap-4">
      {requests.map((request) => (
        <form key={request.id} action={(data) => updateRequest(request.id, String(data.get("status")), String(data.get("internal_notes")))} className="rounded-lg bg-white p-5 shadow-soft">
          <div className="grid gap-3 md:grid-cols-[1fr_auto]">
            <div>
              <h3 className="text-lg font-black">{request.name} · {request.city}, {request.province}</h3>
              <p className="text-sm leading-6 text-zinc-600">{request.email} · {request.whatsapp}</p>
              {request.group_reference ? <p className="mt-1 text-sm leading-6 text-zinc-600"><strong>Referência:</strong> {request.group_reference}</p> : null}
              <p className="mt-2 text-sm font-bold text-zinc-700">{request.support_mode}</p>
              <p className="mt-1 text-sm text-zinc-600">Coordena entrega: {request.can_coordinate_delivery} · Reúne reservas/pagamentos: {request.can_coordinate_orders_payments}</p>
              {request.notes ? <p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-zinc-600">{request.notes}</p> : null}
              <p className="mt-2 text-xs font-bold uppercase text-zinc-500">{new Date(request.created_at).toLocaleString("pt-BR")} · {statusLabels[request.status] ?? request.status}</p>
            </div>
            <select name="status" defaultValue={request.status} className="focus-ring rounded-md border px-3 py-2">
